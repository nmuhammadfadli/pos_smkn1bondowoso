...
public class datasupplier extends JPanel {
    ...
    if (frame instanceof uiresponsive.Mainmenu) {
        ((uiresponsive.Mainmenu) frame).showTambahDataSupplier(); // panggil method di Mainmenu
    }
});
    ...
    JFrame frame = (JFrame) SwingUtilities.getWindowAncestor(this);
    if (frame instanceof uiresponsive.Mainmenu) {
        ((uiresponsive.Mainmenu) frame).showEditDataSupplier();// panggil method di Mainmenu
    }
});
        ...
        // Dummy data
        model.addRow(new Object[]{"SP1001", "Mamba", "Badean", "082233236128"});
