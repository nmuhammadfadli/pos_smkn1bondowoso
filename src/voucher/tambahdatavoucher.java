...
public class tambahdatavoucher extends JPanel {
    ...
    public tambahdatavoucher() {
        ...
        JLabel title = new JLabel("Tambah Data Voucher", SwingConstants.CENTER);
    ...
    private void onSave() {
        ...
        try {
            int newId = dao.insert(v);
            if (newId > 0) {
                JOptionPane.showMessageDialog(this, "Voucher berhasil disimpan.", "Sukses", JOptionPane.INFORMATION_MESSAGE);
                JFrame frame = (JFrame) SwingUtilities.getWindowAncestor(this);
                if (frame instanceof uiresponsive.Mainmenu) ((uiresponsive.Mainmenu) frame).showDataVoucher();
            } else {
                JOptionPane.showMessageDialog(this, "Gagal menyimpan voucher.", "Error", JOptionPane.ERROR_MESSAGE);
            }
        } catch (SQLException ex) {
            JOptionPane.showMessageDialog(this, "Gagal menyimpan voucher:\n" + ex.getMessage(), "Error", JOptionPane.ERROR_MESSAGE);
        }
    }

    // === RoundedTextField & RoundedButton same as design ===
    ...
    class RoundedButton extends JButton {
        ...
        public RoundedButton(String text, Color bg, Color fg) { super(text); backgroundColor = bg; textColor = fg; setFocusPainted(false); setContentAreaFilled(false); setBorderPainted(false); setFont(new Font("Segoe UI", Font.BOLD, 15)); setCursor(Cursor.getPredefinedCursor(Cursor.HAND_CURSOR)); }
        @Override protected void paintComponent(Graphics g) {
    ...
    // PilihGuruFrame: gunakan GuruDAO untuk ambil data guru
    class PilihGuruFrame extends JFrame {
        public PilihGuruFrame(final JTextField targetField) {
            ...
            JTable tabel = new JTable(m);
            tabel.setRowHeight(26);
            tabel.setFont(new Font("Segoe UI", Font.PLAIN, 13));
            JScrollPane scroll = new JScrollPane(tabel);
            panel.add(scroll, BorderLayout.CENTER);

            // load data guru from GuruDAO
            try {
                GuruDAO gdao = new GuruDAO();
                java.util.List<Guru> list = gdao.findAll();
                if (list != null) {
                    for (Guru g : list) m.addRow(new Object[]{ g.getIdGuru(), g.getNamaGuru(), g.getJabatan() });
                }
            } catch (Exception ex) {
                // ignore, show empty
            }
            ...
            btnPanel.add(btnPilih); btnPanel.add(btnCancel);
        ...
        private void styleButton(JButton btn, Color color) {
            btn.setBackground(color); btn.setForeground(Color.WHITE); btn.setFont(new Font("Segoe UI Semibold", Font.BOLD, 14)); btn.setFocusPainted(false); btn.setPreferredSize(new Dimension(120,40));
        }
    }
}
